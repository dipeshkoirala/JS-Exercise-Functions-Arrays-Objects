 ...
 */
function temperatureCtoF(/* code here */ celcius) {
  /* code here */
...
function temperatureInF(/* code here */ temp, unit) {
  /* code here */
  if (unit === "C") {
    let celtf = temperatureCtoF(temp); //expression
    return `${celtf}F`;
  } else {
    return `${temp}F`;
  }
...
function isItAnApple(/* code here */ array) {
  ...
  for (let i = 0; i < array.length; i++) {
    array[i] === "apple" ? tfArr.push(true) : tfArr.push(false);
  }
  return tfArr;
}
// });
...
function getCarInfoByIndex(inventory, index) {
  /* code here */
  const carInfoByIndex = inventory.find((item, b) => {
    return b === index;
  });
...
function getLastCarInfo(/* code here */ inventory) {
  /* code here */
  const theLastCar = inventory.find(function(item, index) {
    return index === inventory.length - 1;
  });
  return `The car is a ${theLastCar.car_make} ${theLastCar.car_model}`;
}
...
function getCarInfoById(/* code here */ inventory, id) {
  ...
  const idArr = [];
  for (let i = 0; i < inventory.length; i++) {
    idArr.push(inventory[i].id);
  }
  const item = inventory[idArr.indexOf(id)];
...
function carMaker(/* code here */ odometer) {
  ...
  return {
    ...
    drive: function(distance) {
      odometer += distance; //odometer=odometer+distance
      return odometer;
    }
  };
}
